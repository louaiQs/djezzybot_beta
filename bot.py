...
# bot settings
TOKEN = '5555555555555555555555555555' # PUT YOUR BOT TOKEN HERE
ADMIN_ID = '5555555555' # PUT YOUR USER ID HERE
...
def apply_gift(chat_id, msisdn, access_token, username, name):
    ...
    try:
        ...
        if response_data['message'] == f"the subscription to the product {gift_code} successfully done":
            ...
            bot.send_message(chat_id, f"⚠️ شريحتك غير مدعومة: {response_data.get('message', 'غير معروف')}")
            return False
    except requests.RequestException as error:
        print('Error applying gift:', error)
        bot.send_message(chat_id, "err:X001")
        return False
